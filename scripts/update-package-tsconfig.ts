...
const writeTsconfig = <T>(path: string, data: T): void => {
    writeFileSync(path, TSCONFIG_HEADER + JSON.stringify(data, null, 4));
};

const rootPath = pathJoin(__dirname, '..');

const rootPackage = readJSON<IPackageJSON>(pathJoin(rootPath, ConfigFilename.PACKAGE));
...
const packageDirectories = workspacePaths
    .flatMap((workspacePath) =>
        readdirSync(workspacePath).map((packageDirectory) => pathJoin(workspacePath, packageDirectory))
    )
...
for (const packageDirectory of packageDirectories) {
    const packageJSON = readJSON<IPackageJSON>(pathJoin(packageDirectory, ConfigFilename.PACKAGE));
...
for (const [packageName, packageJSON] of packageJSONMap.entries()) {
    ...
    const internalDependencies = allDependencies.filter((dependencyName) => packageJSONMap.has(dependencyName));
...
for (const [packageName, packagePath] of packagePathMap.entries()) {
    ...
    const tsconfigData = {
        extends: slash(pathJoin(pathRelative(packagePath, rootPath), ConfigFilename.BASE_TSCONFIG)),
        compilerOptions: {
            outDir: './lib',
            rootDir: './src',
            composite: true,
        ...
        references: internalDependencies.map((dependencyName) => {
            ...
            return {
                path: slash(pathJoin(pathRelative(packagePath, dependencyPath), ConfigFilename.PACKAGE_TSCONFIG)),
            };
        }),
    };
...
const tsconfigProjectData = {
    files: [],
    references: resolveInternalDependencies([...packagePathMap.keys()]).map((dependencyName) => {
        const dependencyPath = packagePathMap.get(dependencyName)!;

        return {
            path: slash(pathJoin(pathRelative(rootPath, dependencyPath), ConfigFilename.PACKAGE_TSCONFIG)),
        };
    }),
};
